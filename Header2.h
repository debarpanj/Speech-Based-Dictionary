...
void load_average_model_live(int digit, int iter)
{
    int i, j;
    char filename[100];
    // lambda folder stores the model values
    sprintf(filename, "lambdasLive/digit%d_A%d_matrix.txt", digit, iter);
    ...
    sprintf(filename, "lambdasLive/digit%d_B%d_matrix.txt", digit, iter);
...
void save_model_live(int k, int iter)
{
    int i, j;
    char filename[100];
    // models are stord inside a folder called lambda
    sprintf(filename, "lambdasLive/digit%d_A%d_matrix.txt", k, (iter + 1));
    ...
    sprintf(filename, "lambdasLive/digit%d_B%d_matrix.txt", k, (iter + 1));
...
void train_live(int time)
{
	...
	for (int i = 0; i <word ; i++) //	10 digit
    {
        reset_model();
        for (int j = 0; j < utterence; j++) // 20 utterances of each training digit
        {
            ...
            for (int bi = 1, k = f; bi<=149 && k < n - 5000; bi++, k += 320)
...
{
    printf("Training... iteration %d\n", time);
    for (int i = 0; i <= 8; i++) //	10 digit
    {
        reset_model();
        for (int j = 0; j <= 29; j++) // 20 utterances of each training digit
        {
            ...
            long double current_frame[320];
            // drops 1st 8000 samples and starts storing
            for (int bi = 1, k = f; bi<=140 && k < n - 8000; bi++, k += 240)
            { // counts number of frame
                num_frames++;
                for (int y = 0; y < 320; y++)
                {
...
int test_livedata()
{
   ...
   int f = findstart(sample, num);
    ...
    for (int bi = 1, k = f;bi<=140 && k < num - 8000; bi++, k += 240)
    {
        num_frames++;
        ...
        for (int j = 1; j <= 12; j++)
        {
            frame_ci[bi][j] = c[j]; // cepstral vector of ith digit is stored
	...
	FILE *fp=fopen("predicted_word.txt","w");

	fprintf(fp,"%d ",maxindex);
	fclose(fp);
	return maxindex;
}
void train_model()
